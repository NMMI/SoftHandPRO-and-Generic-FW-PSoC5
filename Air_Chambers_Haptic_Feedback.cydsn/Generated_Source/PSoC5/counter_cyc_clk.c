...
/*******************************************************************************
* Function Name: counter_cyc_clk_Start
********************************************************************************
*
* Summary:
...
void counter_cyc_clk_Start(void) 
{
    ...
    counter_cyc_clk_CLKEN |= counter_cyc_clk_CLKEN_MASK;
	counter_cyc_clk_CLKSTBY |= counter_cyc_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: counter_cyc_clk_Stop
********************************************************************************
*
* Summary:
...
void counter_cyc_clk_Stop(void) 
{
    ...
    counter_cyc_clk_CLKEN &= (uint8)(~counter_cyc_clk_CLKEN_MASK);
	counter_cyc_clk_CLKSTBY &= (uint8)(~counter_cyc_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: counter_cyc_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((counter_cyc_clk_CLKEN & counter_cyc_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = counter_cyc_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = counter_cyc_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* counter_cyc_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(counter_cyc_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(counter_cyc_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: counter_cyc_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: counter_cyc_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = counter_cyc_clk_GetSourceRegister();
    uint16 oldDivider = counter_cyc_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = counter_cyc_clk_CLKEN & counter_cyc_clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(counter_cyc_clk_DIV_PTR, clkDivider);
                counter_cyc_clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                counter_cyc_clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(counter_cyc_clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((counter_cyc_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: counter_cyc_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(counter_cyc_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: counter_cyc_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    counter_cyc_clk_MOD_SRC |= modeBitMask & (uint8)counter_cyc_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: counter_cyc_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    counter_cyc_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(counter_cyc_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: counter_cyc_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return counter_cyc_clk_MOD_SRC & (uint8)(counter_cyc_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: counter_cyc_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = counter_cyc_clk_GetDividerRegister();
    uint8 oldSrc = counter_cyc_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        counter_cyc_clk_MOD_SRC |= CYCLK_SSS;
        counter_cyc_clk_MOD_SRC =
            (counter_cyc_clk_MOD_SRC & (uint8)(~counter_cyc_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: counter_cyc_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return counter_cyc_clk_MOD_SRC & counter_cyc_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: counter_cyc_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    counter_cyc_clk_PHASE = clkPhase & counter_cyc_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: counter_cyc_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return counter_cyc_clk_PHASE & counter_cyc_clk_PHASE_MASK;
}
