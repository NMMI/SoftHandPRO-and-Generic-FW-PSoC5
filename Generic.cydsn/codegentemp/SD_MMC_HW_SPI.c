...
#define SD_MMC_DEFAULT_SUPPLY_VOLTAGE    (3300u)

/* Max. startup frequency (KHz) */
#define SD_STARTUP_FREQ                   (400u)
...
static char SD_isInited0;

#if (SD_NUMBER_SD_CARDS >= 2u)

    static char SD_isInited1;

#endif /* (SD_NUMBER_SD_CARDS >= 2u) */

#if (SD_NUMBER_SD_CARDS >= 3u)

    static char SD_isInited2;

#endif /* (SD_NUMBER_SD_CARDS >= 3u) */

#if (SD_NUMBER_SD_CARDS == 4u)

    static char SD_isInited3;
...
*
*******************************************************************************/
static void SD_Init(U8 Unit) CYREENTRANT
{
    switch(Unit)
    {
    case 0u:
        ...
        if (SD_isInited0 == 0u)
        {
            ...
            SD_Clock_1_SetDividerValue(BCLK__BUS_CLK__KHZ/SD_STARTUP_FREQ);
        ...
        break;
            ...
            case 1u:
                ...
                if (SD_isInited1 == 0u)
                {
                    ...
                    /* Set the inital clock frequency to 400 KHz */
                    SD_Clock_2_SetDividerValue(BCLK__BUS_CLK__KHZ/SD_STARTUP_FREQ);
            ...
            case 2u:
                ...
                if (SD_isInited2 == 0u)
                {
                    ...
                    /* Set the inital clock frequency to 400 KHz */
                    SD_Clock_3_SetDividerValue(BCLK__BUS_CLK__KHZ/SD_STARTUP_FREQ);
            ...
            case 3u:
                ...
                if (SD_isInited3 == 0u)
                {
                    ...
                    /* Set the inital clock frequency to 400 KHz */
                    SD_Clock_4_SetDividerValue(BCLK__BUS_CLK__KHZ/SD_STARTUP_FREQ);
...
*
*******************************************************************************/
static U8 SD_ReadWriteSPI(U8 Unit, U8 Data) CYREENTRANT
{
    U8 spiData;
    ...
    case 0u:
        ...
        /* Send Data */
        SD_SPI0_WriteTxData(Data);

        /* Wait until all bits are shifted */
        while (0u == (SD_SPI0_STS_SPI_DONE & SD_SPI0_ReadTxStatus()));

        /* Read data */
        spiData = SD_SPI0_ReadRxData();
        ...
        case 1u:
            ...
            /* Send Data */
            SD_SPI1_WriteTxData(Data);

            /* Wait until all bits are shifted */
            while (0u == (SD_SPI1_STS_SPI_DONE & SD_SPI1_ReadTxStatus()));

            /* Read data */
            spiData = SD_SPI1_ReadRxData();
        ...
        case 2u:
            ...
            /* Send Data */
            SD_SPI2_WriteTxData(Data);

            /* Wait until all bits are shifted */
            while (0u == (SD_SPI2_STS_SPI_DONE & SD_SPI2_ReadTxStatus()));

            /* Read data */
            spiData = SD_SPI2_ReadRxData();
        ...
        case 3u:
            ...
            /* Send Data */
            SD_SPI3_WriteTxData(Data);

            /* Wait until all bits are shifted */
            while (0u == (SD_SPI3_STS_SPI_DONE & SD_SPI3_ReadTxStatus()));

            /* Read data */
            spiData = SD_SPI3_ReadRxData();
...
U16 FS_MMC_HW_X_SetMaxSpeed(U8 Unit, U16 MaxFreq)
{
    ...
    if(MaxFreq < SD_STARTUP_FREQ)
    {
        MaxFreq = SD_STARTUP_FREQ;
    }
    else if(MaxFreq > SD_MAX_SPI_FREQ)
    {
        MaxFreq = SD_MAX_SPI_FREQ;
    }
    else
    {
        /* Do nothing */
    ...
    switch(Unit)
    {
        case 0u:
            SD_Clock_1_Stop();
            SD_Clock_1_SetDividerValue((U16)divResult);    /* update the frequency */
            SD_Clock_1_Start();
            break;
        ...
        case 1u:
            ...
            SD_Clock_2_Stop();
            SD_Clock_2_SetDividerValue((U16)divResult);    /* update the frequency */
            SD_Clock_2_Start();
            break;
        ...
        case 2u:
            ...
            SD_Clock_3_Stop();
            SD_Clock_3_SetDividerValue((U16)divResult);    /* update the frequency */
            SD_Clock_3_Start();
            break;
        ...
        case 3u:
            ...
            SD_Clock_4_Stop();
            SD_Clock_4_SetDividerValue((U16)divResult);    /* update the frequency */
            SD_Clock_4_Start();
            break;
...
int FS_MMC_HW_X_SetVoltage(U8 Unit, U16 Vmin, U16 Vmax)
{
    ...
    case 3u:
        ...
        if((Vmin <= SD_MMC_DEFAULT_SUPPLY_VOLTAGE) &&
            (SD_MMC_DEFAULT_SUPPLY_VOLTAGE <= Vmax))
        {
            result = SD_RET_SUCCCESS;
        }
        else
        {
            result = SD_RET_FAIL;
...
*******************************************************************************/
    ...
    void  FS_MMC_HW_X_Read (U8 Unit, U8 * pData, int NumBytes)
    {
        do
        {
            *pData++ = SD_ReadWriteSPI(Unit, 0xff);
        } while (--NumBytes);
    }
#else
