...
uint8 SHIFTREG_ENC_2_initVar = 0u;


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_Start
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  SHIFTREG_ENC_2_initVar - used to check initial configuration, modified on
*  first function call.
*
* Reentrant:
*  No.
...
{
    if(0u == SHIFTREG_ENC_2_initVar)
    {
        SHIFTREG_ENC_2_Init();
        SHIFTREG_ENC_2_initVar = 1u; /* Component initialized */
    }
...
/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_Enable
********************************************************************************
*
* Summary:
...
void SHIFTREG_ENC_2_Enable(void) 
{
    ...
    SHIFTREG_ENC_2_SR_CONTROL |= SHIFTREG_ENC_2_CLK_EN;

    SHIFTREG_ENC_2_EnableInt();
}


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_Init
********************************************************************************
*
* Summary:
...
{
    SHIFTREG_ENC_2_SetIntMode(SHIFTREG_ENC_2_INT_SRC);
}


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_Stop
********************************************************************************
*
* Summary:
...
void SHIFTREG_ENC_2_Stop(void) 
{
    ...
    SHIFTREG_ENC_2_SR_CONTROL &= ((uint8) ~SHIFTREG_ENC_2_CLK_EN);
    SHIFTREG_ENC_2_DisableInt();
}


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_EnableInt
********************************************************************************
*
* Summary:
...
void SHIFTREG_ENC_2_EnableInt(void) 
{
    ...
    SHIFTREG_ENC_2_SR_AUX_CONTROL |= SHIFTREG_ENC_2_INTERRUPTS_ENABLE;
    CyExitCriticalSection(interruptState);
}


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_DisableInt
********************************************************************************
*
* Summary:
...
void SHIFTREG_ENC_2_DisableInt(void) 
{
    ...
    interruptState = CyEnterCriticalSection();
    SHIFTREG_ENC_2_SR_AUX_CONTROL &= ((uint8) ~SHIFTREG_ENC_2_INTERRUPTS_ENABLE);
    CyExitCriticalSection(interruptState);
}


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_GetFIFOStatus
********************************************************************************
*
* Summary:
...
uint8 SHIFTREG_ENC_2_GetFIFOStatus(uint8 fifoId) 
{
    ...
    result = SHIFTREG_ENC_2_RET_FIFO_NOT_DEFINED;
    ...
    if(SHIFTREG_ENC_2_OUT_FIFO == fifoId)
    {
        switch(SHIFTREG_ENC_2_GET_OUT_FIFO_STS)
        {
            case SHIFTREG_ENC_2_OUT_FIFO_FULL :
                result = SHIFTREG_ENC_2_RET_FIFO_FULL;
                break;

            case SHIFTREG_ENC_2_OUT_FIFO_EMPTY :
                result = SHIFTREG_ENC_2_RET_FIFO_EMPTY;
                break;

            case SHIFTREG_ENC_2_OUT_FIFO_PARTIAL :
                result = SHIFTREG_ENC_2_RET_FIFO_PARTIAL;
                break;
...
/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_SetIntMode
********************************************************************************
*
* Summary:
...
{
    SHIFTREG_ENC_2_SR_STATUS_MASK &= ((uint8) ~SHIFTREG_ENC_2_INTS_EN_MASK);          /* Clear existing int */
    SHIFTREG_ENC_2_SR_STATUS_MASK |= (interruptSource & SHIFTREG_ENC_2_INTS_EN_MASK); /* Set int */
}


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_GetIntStatus
********************************************************************************
*
* Summary:
...
{
    return(SHIFTREG_ENC_2_SR_STATUS & SHIFTREG_ENC_2_INTS_EN_MASK);
}


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_WriteRegValue
********************************************************************************
*
* Summary:
...
{
    CY_SET_REG32(SHIFTREG_ENC_2_SHIFT_REG_LSB_PTR, shiftData);
}
    ...
    *******************************************************************************/
                                                                         ...
                                                                         
    {
        ...
        {
            CY_SET_REG32(SHIFTREG_ENC_2_IN_FIFO_VAL_LSB_PTR, shiftData);
    ...
    /*******************************************************************************
    * Function Name: SHIFTREG_ENC_2_ReadData
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        return(CY_GET_REG32(SHIFTREG_ENC_2_OUT_FIFO_VAL_LSB_PTR));
    }
#endif /* (0u != SHIFTREG_ENC_2_USE_OUTPUT_FIFO) */


/*******************************************************************************
* Function Name: SHIFTREG_ENC_2_ReadRegValue
********************************************************************************
*
* Summary:
...
uint32 SHIFTREG_ENC_2_ReadRegValue(void) 
{
    ...
    while(SHIFTREG_ENC_2_RET_FIFO_EMPTY != SHIFTREG_ENC_2_GetFIFOStatus(SHIFTREG_ENC_2_OUT_FIFO))
    {
        (void) CY_GET_REG32(SHIFTREG_ENC_2_OUT_FIFO_VAL_LSB_PTR);
    }

    /* Read of 8 bits from A1 causes capture to output FIFO */
    (void) CY_GET_REG8(SHIFTREG_ENC_2_SHIFT_REG_CAPTURE_PTR);

    /* Read output FIFO */
    result  = CY_GET_REG32(SHIFTREG_ENC_2_OUT_FIFO_VAL_LSB_PTR);
    
    #if(0u != (SHIFTREG_ENC_2_SR_SIZE % 8u))
        result &= ((uint32) SHIFTREG_ENC_2_SR_MASK);
    #endif /* (0u != (SHIFTREG_ENC_2_SR_SIZE % 8u)) */
