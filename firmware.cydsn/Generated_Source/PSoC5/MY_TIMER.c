...
{
   #if(MY_TIMER_UsingFixedFunction)
       return ((uint16)CY_GET_REG16(MY_TIMER_PERIOD_LSB_PTR));
   #else
       return (CY_GET_REG16(MY_TIMER_PERIOD_LSB_PTR));
   #endif /* (MY_TIMER_UsingFixedFunction) */
}
...
void MY_TIMER_WritePeriod(uint16 period) 
{
    ...
    #else
        CY_SET_REG16(MY_TIMER_PERIOD_LSB_PTR, period);
...
{
   #if(MY_TIMER_UsingFixedFunction)
       return ((uint16)CY_GET_REG16(MY_TIMER_CAPTURE_LSB_PTR));
   #else
       return (CY_GET_REG16(MY_TIMER_CAPTURE_LSB_PTR));
   #endif /* (MY_TIMER_UsingFixedFunction) */
}
...
void MY_TIMER_WriteCounter(uint16 counter) 
{
    ...
    #else
        CY_SET_REG16(MY_TIMER_COUNTER_LSB_PTR, counter);
